...
def maybe_separate_query_ids(dataset, queries, qrels, separate_query_ids):
    if not separate_query_ids or not dataset.startswith("situatedqa-"):
        return queries, qrels
    remapped_queries = {f"{SITUATED_QUERY_PREFIX}{qid}": text for qid, text in queries.items()}
    remapped_qrels = {f"{SITUATED_QUERY_PREFIX}{qid}": rels for qid, rels in qrels.items()}
    return remapped_queries, remapped_qrels

def load_dataset(dataset: str, separate_query_ids: bool = False):
    ...
    if dataset.startswith("situatedqa-"):
        ...
        queries, qrels = maybe_separate_query_ids(dataset, queries, qrels, separate_query_ids)
...
def prepare_dataset(dataset: str, size_percent: float = 100.0, fix_qrels_headers: bool = False, seed: int = DEFAULT_RANDOM_SEED, separate_query_ids: bool = False):
    if fix_qrels_headers:
        ensure_qrels_headers([dataset])
    corpus, queries, qrels = load_dataset(dataset, separate_query_ids=separate_query_ids)
