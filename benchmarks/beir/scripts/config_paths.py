...
def _derive_beir_datasets(config: dict, project_root: Path):
...
if "bench_root" in _paths_cfg:
    BENCH_ROOT = _resolve_from_project_root(_PROJECT_ROOT, _paths_cfg["bench_root"])
else:
    BENCH_ROOT = (_PROJECT_ROOT / "benchmarks" / "beir").resolve()

if "normalized_datasets_root" in _paths_cfg:
    DATASET_ROOT = _resolve_from_project_root(_PROJECT_ROOT, _paths_cfg["normalized_datasets_root"])
elif "datasets_root" in _paths_cfg:
    DATASET_ROOT = _resolve_from_project_root(_PROJECT_ROOT, str(Path(_paths_cfg["datasets_root"]) / "normalized_datasets"))
else:
    DATASET_ROOT = (_PROJECT_ROOT / "datasets" / "normalized_datasets").resolve()

DATASETS = _derive_beir_datasets(_CONFIG, _PROJECT_ROOT)
