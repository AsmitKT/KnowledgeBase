...
def build_indexes(config,dataset_name):
    ...
    bm.build(corpus)
    dr=DenseRetrieval(config['hybrid']['dense']['model_name'],config['hybrid']['dense']['batch_size'],normalize=config['hybrid']['dense']['normalize'])
    dr.build(corpus)
...
def search_query(config,dataset_name,query_text,top_k):
    ...
    lists={'bm25':bm_res,'dense':dense_res,'ann':ann_res,'meta':meta_list}
    weights={'bm25':config['hybrid']['fusion']['bm25_weight'],'dense':config['hybrid']['fusion']['dense_weight'],'ann':config['hybrid']['fusion']['dense_weight'],'meta':config['hybrid']['fusion']['metadata_weight']}
    fused=rrf_fuse(lists,weights,config['hybrid']['fusion']['rrf_k'],top_k)
    return fused

def evaluate(config,dataset_name,split,top_k):
    corpus,queries,qrels=load_dataset(config,dataset_name)
    run={}
    for q in queries:
        qid=q.get('id') or q.get('query_id') or q.get('_id')
        res=search_query(config,dataset_name,q.get('text') or q.get('question'),top_k)
